...
2. **No citation points at nothing.** Citations are written symbolically (`[@chauhan]`) in the
   chapter files and resolved to numbers _after_ renumbering, so adding or removing an entry can
...
## 8. One more silent corruption, found on the way out

`full.md` is generated, but `prettier --check` runs over the whole repository, so the committed
file had been reformatted after generation. Two things had been damaged, and both were already in
the file the owner was about to take out for formatting:

- **Formula (2.15)** read `V(π) = E(x~~D) E(a~~π(·|x))`. Prettier parses `~D) E(a~` as GitHub
  strikethrough and re-emits it in the canonical double-tilde form. The expectations
  `E(x~D)` and `E(a~π(·|x))` are now restored.
- **The code listings in §4.2 and §4.4** had lost their indentation and their aligned trailing
  comments, because prettier collapses runs of spaces inside a paragraph.

`docs/thesis/text/full.md` is now in `.prettierignore` with the reason written next to it, and
`verify-docx.py` asserts (2.15) in its correct form and fails on the strikethrough artefact. The
generator is once again the sole owner of the file: `assemble.py` produces it byte for byte and
re-running it leaves the tree clean.
