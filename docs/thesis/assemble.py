...
ROLLUP_EDITS: list[tuple] = [
    ...
    ("10.9", 1, "replace", "[МІСЦЕ ДЛЯ РИСУНКА Б.1]", None),
]
...
LITERAL_EDITS: list[tuple] = [
    ...
    ("append", "Незалежно від цього, для забезпечення оцінюваності",
     ...
     "на завантажених, пораховані на коді придатності, а не спостережені (підрозділ 5.5.1).",
     "§2.3 slice frequency"),
...
def fence_json_example(blocks: list[dict]) -> None:
    """Додаток Ж: the draft carries the /plan response as one paragraph per line; as prose it
...
def listing_solver_parameters(blocks: list[dict]) -> None:
    ...
    for b in blocks:
        ...
        # one scaled unit is the stability bonus: the deployed OBJECTIVE_SCALE is 10 000, so
        # 1e-4 in weight units is exactly one unit (the draft scaled by 1 000)
        w = lines.index("        weight_terms.append((int(w * 1000), lit))")
        lines[w] = "        weight_terms.append((int(w * 10_000), lit))"
...
def build() -> tuple[list[dict], dict]:
    ...
    cell_set(blocks, 2, 2, 1,
             "Інфраструктурно-незалежний контейнер; постачальник із безстроковим безоплатним рівнем («Always Free»); задокументований шлях платної міграції (підрозділ 3.3)",
    ...
    para_append(blocks, "UC-03. Генерація денного плану",
                "Альтернатива: день без робочого вікна – запиту не надсилається, план не зберігається, вечірнє сповіщення-запрошення спланувати наступний день не планується; екран пояснює причину.",
    ...
    run_sub(blocks, "UC-07. Ручне перевизначення як навчання",
            ...
            "Виклик «Перенести…» на блоці: вибір нового часу на сітці 15 хв → оновлення розміщення → парний сигнал (негатив для початкового інтервалу 0,1 / слабкий позитив для цільового 0,7, одна пара на розміщення; цільовий контекст обчислюється на сервері тим самим кодом сітки та ознак, що й під час генерації плану)",
    ...
    run_sub(blocks, "Збірка та випуск: EAS Build",
            ...
            "Збірка та випуск: профілі EAS Build і EAS Update підготовлено та перевірено; жодного подання до магазинів не виконано (підрозділ 5.2.3): роботу розповсюджують без облікових записів магазинів – збіркою APK для Android; каналу для учасників з iOS немає, бо TestFlight потребує платного членства. EAS Build для магазинних бінарників",
    ...
    # --- second pass (2026-09-14): dangling contrasts, broken cross-references, hosting residue --
    run_sub(blocks, "у якому кожен фрагмент задачі отримує не більше одного інтервалу", ", а кожен інтервал – не більше однієї задачі", "", "§2.1 duplicated constraint")
    run_sub(blocks, "де C – множина контекстних бакетів", "клас відносної позиції), У реалізації", "клас відносної позиції). У реалізації", "§2.2 sentence seam")
    run_sub(blocks, "Варіант LinUCB (експериментальне плече для ДП3)", "Варіант LinUCB (експериментальне плече для ДП3)",
            "Варіант LinUCB (альтернативна стратегія дослідження, реалізована в сервісі; окремим плечем у протокол польового дослідження не входить)", "§2.3 LinUCB arm")
    run_sub(blocks, "або обчислити LinUCB-оцінки (2.10) для відповідного плеча", "для відповідного плеча", "якщо обрано цю стратегію", "Алгоритм 2.1 LinUCB")
    run_sub(blocks, "зафіксувати пропенсіті p = ε/m.", "зафіксувати пропенсіті p = ε/m.",
            "зафіксувати пропенсіті p = ε/|A_m(x)|, де A_m(x) – досяжні та вільні на момент розіграшу бакети з top-m.", "Алгоритм 2.1 propensity")
    run_sub(blocks, "узгоджено з трирівневим UX холодного старту", "узгоджено з трирівневим UX холодного старту («режим навчання» в інтерфейсі)",
            "узгоджено з UX холодного старту («режим навчання» в інтерфейсі, таблиця 1.3)", "§2.5.2 cold-start UX")
    run_sub(blocks, "становить зміст дослідницького питання ДП2", "і виконується абляційними аналізами на журналах польового дослідження",
            "і потребує абляційних аналізів на журналах польового дослідження, яких у цій роботі не виконано (підрозділ 1.6)", "§2.7 ДП2 tense")
    run_sub(blocks, "Евристичний резерв (NFR-R2) розміщено саме в Edge Function",
    ...
    run_sub(blocks, "Розклади pg_cron запускають нічну атрибуцію та тригери планування нового дня.",
            ...
            "Розклади pg_cron запускають нічну атрибуцію та п'ятихвилинну пересинхронізацію календарів (підрозділ 3.6).", "§4.3 pg_cron")
    run_sub(blocks, "EAS Update для OTA-доставлення", "EAS Build для магазинних бінарників", "Профіль EAS Build призначено для магазинних бінарників", "§4.6 EAS fragment (1)")
    ...
    cell_sub(blocks, "ворклети Reanimated; парний сигнал контекстів", "вибір часу «Перенести…»; парний сигнал контекстів", "табл. Е.1 FR-25 mechanism")
    para_insert_after(blocks, "лінійний алгоритм верхніх довірчих меж для контекстних бандитів",
                      "M-01 / M-02 – вимоги до вимірювання: точне пропенсіті рандомізованого зрізу та прапорець одночасного зовнішнього конфлікту",
                      "ПЕРЕЛІК: M-01 / M-02")
