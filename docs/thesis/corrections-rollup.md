...
> Cloud у регіоні ЄС (підрозділ 3.3). Сама подія є результатом роботи: залежність від
...
> поза вікном отримує r = 0,3, і лише під час авторитетної нічної атрибуції. Явне відхилення
> блоку користувачем (подія `block_skipped`) дає r = 0 негайно й переводить рядок у стан
> `rejected` – статус із набору перегляду плану, який клієнт має право записати й на сервері.
> Непрожитий блок (подія `lapse_observed`) переводить рядок у стан `lapsed`, і миттєвий шлях для
> нього не спрацьовує ніколи: локально статус ставить лінивий сканер одразу, на сервері – лише
> нічна атрибуція, яка тоді ж і породжує кортеж r = 0. Окремого стану `skipped` у схемі немає, а
> типи подій `block_skipped` і `lapse_observed` лишаються різними, щоб зареєстрований код PAR
> ніколи не сплутав власне спостереження клієнта з фактом. Пізніше «насправді я це зробив» протягом
