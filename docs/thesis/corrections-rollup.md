...
**(h) The seventh dimension as a costed quantity — П (§3.3; the two closing paragraphs of §1.4 (b)
as approved).** Placed after (g); see the 2026-09-15 record under §1.4 (b) for why they stand in
ВСТУП rather than in §1.4.

> «Внеском роботи щодо сьомого виміру – розгорнутого польового оцінювання (D7 у матриці підрозділу
> 1.4) – є не його закриття, а перетворення його з невизначеної перспективи на величину з ціною.
> Протокол розроблено, інструментовано й перевірено наскрізно на розгорнутій системі: точні
> пропенсіті кожного рядка, перемикання плечей, блокова рандомізація, показник дотримання з фактів,
> стенд офлайн-оцінювання, перевірений на відомій істині. А симуляційне оцінювання (розділ 6) уперше
> дає відповідь на питання, скільки коштувало б його виконати саме для цього класу задач: від 21 до
> понад 120 завершених учасників залежно від властивостей популяції, причому понад 120 – у 48 із 75
> розглянутих світів, тобто набір щонайменше 172 осіб (120 / 0,7 за припущеного відсіву 30 %) на
...
> зареєстрованій верхній межі навантаження. Це число – і є відповідь на питання, чому жодна праця не
> займає всіх семи вимірів матриці підрозділу 1.4.
>
> Вимірювання цього ресурсу є самостійним результатом: попередні праці, що обіцяли польове
> оцінювання планувальника, обсягу вибірки з властивостей задачі не виводили.»
...
> порівняно експериментально, одне: «найраніший вільний слот» (підрозділи 5.7 і 6.4).»
...
**(b) The gap paragraph after the table — С, replaced by two paragraphs.** The rebuilt argument does
not claim a completed row. It asks **why** the row is empty, answers with a property of the problem
class; the two paragraphs that put a number on that property are 4.3 (h).
...
>
> Сьомий вимір – розгорнуте польове оцінювання – робота не закриває, і це не випадковість окремого
> проєкту, а властивість самого класу задач. Варто подивитися, які напрями в матриці мають D7. Його
> мають ті, чия інтервенція дешева в розгортанні, а результат – майже миттєвий (контекстні бандити
> на новинних стрічках, бандити для моментів надсилання сповіщень, своєчасні адаптивні інтервенції),
> і календарні асистенти, які розгортають правило, а не навчання (D3, D4 відсутні). Його не мають
> ті, хто поєднує жорсткі ресурсні обмеження з онлайн-навчанням на поведінці (комбінаторні
> напівбандити) – і саме це поєднання є ядром цієї роботи за D2+D3. Порожній рядок у матриці – не
> прогалина, яку ніхто не помітив, а наслідок двох властивостей самої задачі, які підрозділ 1.6
> кладе в основу постановки. Винагорода є відкладеним поведінковим результатом, тож одиницею
...
Two records, both 2026-09-15. (1) The payload used to carry four paragraphs; its two closing ones —
the seventh dimension as a costed quantity, and «вимірювання цього ресурсу є самостійним
результатом» — had been landing in ВСТУП, not here: the assembler placed each extra paragraph after
the first paragraph whose opening 60 characters matched, and paragraph 2's opening also opens a
sentence of 4.3 (g). The second pass of 2026-09-14 saw them there and expanded them for that
position (D1–D7 and «рядок» are introduced only in §1.4), so the intro placement is the reviewed
one; they are now 4.3 (h), placed there by name, and extra paragraphs follow their head by position.
(2) Owner's submission report, item 3: the sentence «властивості, які роблять рекомендування часових
інтервалів складним, є тими самими властивостями, які роблять польове оцінювання дорогим» leaves the
middle of paragraph 2 for §1.6, where the contribution is formulated; paragraph 2 keeps the two
mechanisms and points forward.
...
> «По-перше, безоплатні рівні хмарних платформ покривають базу даних, інференс і навчання за
> нульової вартості на ранньому масштабі [60, 30, 23]. Для даних, автентифікації та крайових функцій
> це Supabase у регіоні ЄС; для сервісу рекомендацій і нічного навчання – безоплатна віртуальна
> машина Oracle Cloud у Марселі в контейнері, не прив'язаному до постачальника, обрана за чотирма
> ознаками: регіон ЄС відповідає вимогам GDPR до місця обробки, два реальні ядра відповідають
> потребам CP-SAT, відсутність холодного старту відповідає бюджету затримки NFR-P1, а вартість
> нульова (підрозділ 3.3). Залежність від безоплатних рівнів лишається окремим ризиком дослідницьких
> систем, і його пом'якшує саме інфраструктурно-незалежний контейнер (таблиця 1.3). По-друге, ринок
> підтвердив готовність платити за автоматичне планування, залишивши порожнім квадрант недорогих
> рішень, що навчаються. По-третє, після поширення великих мовних моделей користувачі очікують, що
> програмне забезпечення адаптується до них. Приватність забезпечено обробкою в ЄС, RLS,
> мінімізацією даних та стиранням; перенесення персонального ранжувальника на пристрій (ONNX
> Runtime) віднесено до перспектив.»
...
> пропенсіті) від першого дня створює субстрат, придатний для незміщеного офлайн-оцінювання; (3)
> петля зворотного зв'язку, яка не дає накопичувального ефекту, який можна виміряти: приріст
> персоналізації становить 0,3–1,4 в. п. за чотири тижні за інформативного приору (розділ 6), тож
> перевага навчання є не наслідком часу, а наслідком наявності індивідуальної варіації в популяції
> користувачів.»
...
| Ліміти безоплатних тарифів у разі зростання   | «Архітектура з задокументованим шляхом платної міграції…»                 | «Інфраструктурно-незалежний контейнер; постачальник із безстроковим безоплатним рівнем («Always Free»); задокументований шлях платної міграції (підрозділ 3.3)»                                  |
...
> «Стан відповідей на дослідницькі питання, отриманий у цій роботі, такий. ДП1 – відповідь отримано
> частково: навчена політика порівнюється з детермінованим правилом «найраніший вільний слот» на
> сітці змодельованих світів, і межу переваги локалізовано (підрозділ 6.4); порівняння з
> неперсоналізованими популяційними середніми як окремим плечем не проводилося, а виміряний внесок
> популяційної таблиці приорів становить ±0,4 в. п. ДП2 – відповіді не отримано: зважування сигналів
> зворотного зв'язку потребує реальних поведінкових даних, абляцій стратегій атрибуції не
> проводилося; правила атрибуції реалізовано й покрито тестами. ДП3 – відповідь отримано частково:
> виміряно ціну рандомізованого зрізу (0,03–17 в. п. на рандомізованих блоках, ≈ 0–4 в. п. на рівні
> плеча) і розкладено внесок дисперсії семплінгу Томпсона; порівняння «дослідження проти чистої
> експлуатації» не проводилося, а довіра користувача в симуляції не вимірюється. ДП4 – відповідь
> отримано: сімейство оцінювачів відновлює істинну цінність політики на відомій істині, а незважений
> replay виявився зміщеним за змінного розміру рандомізованого зрізу, тому на зрізі використовується
> зважений replay (підрозділ 6.1).»

Owner's submission report, 2026-09-15, items 3, 7 and 4 — placement only, no claim changes. Item 3:
the strongest sentence of the work (§1.4 (b), paragraph 2) belongs where the contribution is
formulated. Item 7: §5.8 presents the reproducibility package as housekeeping, while it is the
strongest part of the methodology — one sentence here. Both go in one paragraph before the status
paragraph above (the adversarial pass cut a second sentence that derived the package from the
problem property — a new argument, not a restatement):

> «Постановку задачі визначає властивість, яку виявила порівняльна матриця підрозділу 1.4:
> властивості, які роблять рекомендування часових інтервалів складним, є тими самими властивостями,
> які роблять польове оцінювання дорогим. Відкладена поведінкова винагорода робить одиницею
> спостереження тиждень, а не клік, а комбінаторна зв'язаність призначень робить інтервенцією цілий
> розклад; тому внесок роботи щодо сьомого виміру полягає не в його закритті, а в перетворенні його
> на величину з ціною. До методики належить і пакет відтворюваності: гіпотези, очікувані напрями,
> план аналізу та критерії спростування зареєстровано в системі контролю версій із перевірними
> часовими мітками, що передують появі коду дослідження, синтетичний набір подій згенеровано відомою
> моделлю виконання, а кожну таблицю офлайн-оцінювання відтворює однокомандний стенд (підрозділ
> 5.8).»

Item 4: §5.1 names the evaluation boundary strongly, but on page forty-one, after the reader has
already asked «то ви нічого не перевірили на людях?». One sentence, the last paragraph of §1.6 —
scoped as §5.1 scopes it, «оцінювання методу виконано в симуляції», because Розділ 6 also carries
the hardware measurements of §6.6:

> «Межу оцінювання варто назвати вже тут, а не в розділі 5: польове дослідження в цій роботі не
> виконувалося – протокол спроєктовано, інструментовано й перевірено наскрізно на розгорнутій
> системі, а його виконання лежить поза межами роботи, – тож оцінювання методу виконано в симуляції,
> яка не перевіряє жодного твердження про поведінку людей (підрозділ 5.1).»
...
>
> Друге вимірювання стосується того, на що витрачається цей бюджет. На класі задач, який виявила
> апаратна перевірка (підрозділ 6.6) – взаємозамінні задачі під двома дедлайнами, – розв'язувач у 12
> із 15 запитів доходив до ліміту, і телеметрія показала чому: перше допустиме розв'язання
> з'являлося за 10–11 мс, останнє поліпшувальне – за 54 мс (p50), а решту секунди займало доведення
> оптимальності, яке не завершується: відносний розрив меж лишався 0,38–1,21. Тому до ліміту часу
> додано критерій зупинки за відносним розривом 0,01 та зупинку за відсутності поліпшення протягом
> 0,3 с. Втрата від обриву становить ≤ 0,3 % значення цільової функції (максимум 4,41 % за
> найкоротшого вікна) – на порядок менше за розкид самого семплінгу Томпсона на тій самій задачі
> (5,6–19,2 вагових одиниці між сидами): план є реалізацією з апостеріорного розподілу в будь-якому
> разі. Після розгортання зміни медіана часу розв'язання знизилася з 1003 до 400 мс, а частка
> евристичних резервів – з 1/10 до 0/10 на пристрої та з 1/36 до 0/36 у наскрізному прогоні.
>
...
> «Одне впорядкування таблиці є неперевіреним припущенням: для ранкових типів вона ставить AF вище
> за MD (DM 0,55 проти 0,50; MM 0,58 проти 0,52), тоді як генеративна модель, використана в
> замкненому симуляційному дослідженні, має зворотний порядок. Саме ця клітинка пояснює частину
> втрати ранкових типів у підрозділі 6.3 і зафіксована як припущення, а не як факт.»

**(b) §2.5.3–§2.5.4 — Ф (items 40, 60 g, 60 o).** Add after the fold-in and the empirical-Bayes
paragraphs:

> «Внесок обох приорних механізмів – популяційної таблиці та колаборативного шару – у роботі
> виміряний лише частково: у симуляційному оцінюванні популяційна таблиця приорів проти плоского
> приору дає ±0,4 в. п. у п'яти світах абляції, а колаборативний шар (ALS + кластери) у симуляції
> участі не брав і на реальних даних не запускався. Хронотипне опитування й популяційні приори
> лишаються стартовим наближенням, а не механізмом переваги.»
...
> Beta-комірці (вага α₀ + β₀), «підтверджую» → успіхи, «спростовую» → невдачі, із 28-денним
> напіврозпадом – тим самим, що застосовується до свідчень (підрозділ 2.7); у силі лишається лише
> остання мітка комірки, а кожна доставка мітки запускає повну перебудову стану зі збережених
> кортежів винагород і міток. Мітки впливають лише на енергетичну модель: стан лінійного бандита (A,
> b) мітки не бачить, бо в мітки немає вектора ознак.»
...
> випадково над множиною кандидатів, а винагороди не залежать від логера. На рандомізованому зрізі
> вибір рівномірний у межах A_m(x), і аргумент незміщеності [38] застосовний окремо для кожного
> контексту. Проте розмір зрізу не сталий: |A_m(x)| ∈ {2, 3, 4}, а рядок потрапляє в збіг з
> імовірністю 1/|A_m(x)|, тож рядки з малим зрізом збігаються вдвічі частіше за рядки з великим, і
> незважений replay оцінює цінність на розподілі контекстів, переваженому вагами 1/|A_m(x)|. Для
> політики, цінність якої корелює з розміром зрізу, це дає систематичне зміщення: виміряно −0,6 в.
> п. на оракульній політиці та +0,7 в. п. на антиоракульній (3,2 та 4,6 стандартні похибки
> Монте-Карло на 200 повтореннях по 1 000 рядків), що збігається із замкненою формою (−0,55 та +0,54
> в. п.). Оцінювачі IPS, SNIPS і DR зважують кожен збіг на |A_m(x)| і лишаються незміщеними.
> Нормативне рішення роботи: на зрізі replay подається лише поряд зі SNIPS та DR, або його збіги
> зважуються на |A_m(x)| – що тотожно IPS. Зміщення є прямим наслідком проєктного рішення про
> правило придатності, яке допускає від двох до чотирьох досяжних бакетів заради обсягу даних:
> вимога рівно чотирьох дала б ≈ 615 рядків із незміщеним replay (ESS ≈ 154) проти ≈ 930 рядків із
> IPS (ESS ≈ 310); ціну сплачено лише незваженим оцінювачем, і її повернено зважуванням.»
...
> «Базові правила атрибуції такі. Сесії, що почалися в межах ±15 хв від початку інтервалу, належать
> блоку: r = 1, якщо сумарний сфокусований час у вікні становить ≥ 50 % запланованого, інакше r
> дорівнює цій частці. Виконання без фокус-сесії зараховується як внутрішньовіконне, якщо момент
> виконання лежить у межах [початок − 15 хв, кінець + 15 хв]. Виконання того самого дня поза вікном
> отримує r = 0,3, і лише під час авторитетної нічної атрибуції. Явне відхилення блоку користувачем
> (подія `block_skipped`) дає r = 0 негайно й переводить рядок у стан `rejected` – статус із набору
> перегляду плану, який клієнт має право записати й на сервері. Непрожитий блок (подія
> `lapse_observed`) переводить рядок у стан `lapsed`, і миттєвий шлях для нього не спрацьовує
> ніколи: локально статус ставить лінивий сканер одразу, на сервері – лише нічна атрибуція, яка тоді
> ж і породжує кортеж r = 0. Стани `rejected` і `lapsed` навмисно різні, як і типи подій
> `block_skipped` та `lapse_observed`, щоб зареєстрований код PAR ніколи не сплутав власне
> спостереження клієнта з фактом. Пізніше «насправді я це зробив» протягом 7 днів переписує
> збережений пропуск на r = 1 і запускає повну перебудову стану, зберігаючи початковий момент
> атрибуції для загасання. Межу 23:55 місцевого часу обчислює SQL у часовому поясі користувача, і її
> перевірено на переходах літнього часу. Кортежі винагород спершу зберігаються, а потім
> доставляються сервісу з маркером підтвердження: недоступність сервісу затримує навчання, але
> нічого не втрачає (повторна доставка ідемпотентна). Оцінювач тривалості (UC-06 A2) – EWMA з α =
> 0,3 за співвідношенням сфокусованих і оцінених хвилин на завершених сесіях – обчислюється в
> крайовій функції та застосовується до оцінки задачі для обох рушіїв планування – евристичного та
> навченого (підрозділ 4.3) – після трьох сесій, з обмеженням множника до [0,5; 2]; частиною стану
> бандита він не є. Витіснення блоку зовнішньою подією календаря винагороди не породжує; винагорода,
> яку не можна однозначно приписати блоку, позначається неоднозначною і виключається з оновлення
> бандита; лічильники Beta-комірок загасають із напіврозпадом 28 днів.»
...
> «Маркування «резервний план» прив'язане до причини (тайм-аут або недосяжність сервісу), а не до
> тегу рушія: плани плеча A в дослідженні також мають `engine = heuristic`, і маркування за рушієм
> зруйнувало б засліплення. Дні, у які стався збій сервісу, виключаються з аналізу. Бюджет резерву
> становить 1,9 с.»
...
> «сервіс рекомендацій – FastAPI на Python 3.12 [21] у контейнері на віртуальній машині Oracle Cloud
> «Always Free» (Ampere A1, 2 OCPU / 12 ГБ, регіон eu-marseille-1) з реєстром моделей у Supabase
> Storage (ЄС); розв'язувач OR-Tools CP-SAT [25]; матрична факторизація – implicit [31];
...
> Sentry [57] у регіоні ЄС; неперервна інтеграція – GitHub Actions [23]. Послідовнісна модель
> SASRec-lite, он-девайс ONNX-ранжувальник і текстові вкладення MiniLM віднесені до перспектив.
> Версії інструментів фіксуються на момент реалізації (jest 29.7, ESLint 10, TypeScript 5.9.3 –
> останнє через діапазон сумісності openapi-typescript).»
...
> «Межа «не пізніше ніж за 5 хв» є серверною властивістю: push-канал Google (секунди, типово) з
> підстрахуванням п'ятихвилинним завданням pg_cron, яке пересинхронізовує будь-який під'єднаний
> календар, не синхронізований останні 5 хв, і продовжує канали. Пристрій дізнається про новий стан
> під час наступного переходу на передній план або 60-секундного опитування: offline-first клієнт є
> узгодженим із часом, і межа захищає шлях винагород – витіснення фіксується на сервері до того, як
> надійдуть факти.»
...
> розташованому в третій країні, – приклад 10 Настанов. Оскільки контролер перебуває в Україні, для
> якої рішення про адекватність відсутнє, кожен експорт із європейських обробників на машину
> дослідника – включно з псевдонімізованими журналами подій – є таким передаванням. Учасники
> набираються в Україні (університетські розсилки та місцеві спільноти продуктивності), проте
> резидентів ЄС/ЄЕЗ це не виключає. Систему спроєктовано за суворішим європейським режимом незалежно
> від цього: усі дані учасників зберігаються й обробляються в регіонах ЄС – Supabase (eu-west-1) та
> віртуальна машина Oracle Cloud у Франції (eu-marseille-1), на якій виконуються також навчання
> моделей і аналіз дослідження. Дослідник, установлений в Україні, отримує лише анонімні агрегати;
> випадковий адміністративний доступ до окремих записів покривається явною згодою учасника (ст.
> 49(1)(a) GDPR; за Законом України № 2297-VI, ст. 29, держави ЄС/ЄЕЗ є адекватними напрямами
> передавання). Якщо до дослідження долучиться резидент ЄС/ЄЕЗ, GDPR застосовується до дослідника за
...
> `deletion_audit` і часом завершення. Електронна пошта не може бути універсальним каналом
> підтвердження, бо анонімні облікові записи не мають адреси, а ст. 12(3) вимагає інформувати «без
> невиправданої затримки», що синхронне підтвердження в застосунку задовольняє найкраще. Каскадне
> видалення виконується синхронно (виміряно 78–151 мс на боці сервера через діалоги застосунку) у
> межах законодавчої межі 30 днів. Анонімні облікові записи очищаються після 30 днів неактивності
> (жодного входу, жодної події), а не через 30 днів без конверсії: активна пробна експлуатація не
> знищується. Двадцятичотиримісячне вікно зберігання сирих подій відлічується від завершення
> дослідження і виконується завданням архівування, а не підмітанням на видалення.»
...
> місце або список прокручується до нього з «приземленням» картки. Критерій єдиний: рух має сказати,
> що змінилося і куди воно поділося; решта переходів (поява плану, повернення в Inbox, шторки)
> навмисно миттєві, бо рух, який каже лише «щось сталося», є оздобленням. Ручне перевизначення
> виконується вибором часу («Перенести…») і дає пару навчальних сигналів – негативний для
> початкового контексту та слабко-позитивний для цільового; жест перетягування віднесено до
> перспектив. Під reduced motion переходи проходять тим самим шляхом із нульовою тривалістю.
> Прощення за замовчуванням: деструктивні дії скасовні 6 с; «пропустити» ніколи не буває червоним.
> Типографіка: статичні накреслення Inter (400/500/600/700) – React Native не надає доступу до осей
> змінних шрифтів; JetBrains Mono з табличними цифрами для таймерів. Не всі кольорові пари
> відповідають WCAG 2.2 AA як текст: акцентні кольори (2,1–2,98:1) використовуються лише як заливки
> з текстовою альтернативою, для основного тексту застосовано токени `textPrimary`/`textSecondary`,
> а для деструктивних підписів використовується окремий токен `danger-text` (6,47:1 у світлій темі,
> 6,10:1 у темній).»
...
> журналює пару сигналів – негативний для початкового контексту та слабко-позитивний для цільового –
> разом зі знімками обох контекстів. Дві взаємодії на стрічці «Сьогодні» анімовано ворклетами
> react-native-reanimated у потоці інтерфейсу (підрозділ 3.8); теплокарту енергії намальовано
> нативними View, кільце фокус-таймера – канвасом react-native-skia.
>
> Природномовне швидке додавання (FR-11) виконується на пристрої двома розбирачами: власна граматика
> тривалостей працює першою й маскує свої фрагменти, після чого chrono-node розбирає дати. Порядок є
> суттєвим: chrono тлумачить «2 год» як відносний час, тобто дедлайн через дві години, і без
> маскування кожна тривалість тихо перетворювалася б на дедлайн. Розбір працює двома мовами –
> англійською та українською (`chrono.uk`); нерозпізнана фраза лишається частиною назви, і
> застосунок каже про це прямо. Двомовність тут – не деталь реалізації, а знята вимірюванням вимога
> специфікації: вона обмежувала розбір англійською, вважаючи український розбирач відсутнім, а
> перевірка закріпленої версії бібліотеки показала його в повній підтримці, і обмеження знято
> (таблиця 6.7, підрозділ 6.8). Прев'ю-чип показує назву, тривалість і дедлайн, а решті полів FR-10
> присвоюються неявні значення за замовчуванням – категорія «admin», пріоритет 2, 30 хв за
> відсутності тривалості, – усі редаговані в аркуші задачі. Чипи уточнення відображаються лише для
> випадку, коли названо сам день тижня без дати; решту двох видів неоднозначності (кілька дат,
...
actual contract. One edit inside the code block:

- `await enqueue(db, skipEvent(r, contextSnapshot(r, now)));` →
  `await enqueue(db, lapseObservedEvent(r, contextSnapshot(r, now)));`

The §4.2 prose named the same event «skip»; it now says `lapse_observed` too (a `run_sub`).

**Comments are out of the listings (owner, 2026-09-13): the code stays, the explanation goes to
prose.** The two paragraphs below carry what the trailing comments said (the monotonic id, the
optimistic local apply, the reactive re-render, the neutral lapsed styling) and the rationale that
was previously addressed «to the caption or the surrounding text». The first goes after the sentence
that announces the listing; the second after the listing, before §4.3.

> «У лістингу функція enqueue у межах однієї локальної транзакції оптимістично застосовує операцію
> до локальних таблиць і записує її до outbox разом із клієнтським монотонним ідентифікатором і
> знімком base_version зміненого рядка; реактивні запити до локальної БД перемальовують інтерфейс
> одразу після фіксації транзакції, не чекаючи на мережу. Функція lapseScanOnForeground є сканером
> лінивої фіксації пропусків: рекомендації, чий інтервал завершився без події виконання, отримують
> локальний статус lapsed із нейтральним стилюванням, а до черги додається подія lapse_observed зі
> знімком контексту.»

> «Клієнт позначає пропуск локально й журналює власне спостереження (`lapse_observed`); явний
> пропуск користувачем – інша подія (`block_skipped`). Розрізнення є нормативним: зареєстрований
> код PAR і відображення винагород ніколи не повинні сплутати спостереження клієнта з дією
> користувача. Статус lapsed лишається локальним: авторитетну атрибуцію виконує нічне завдання, а
> станами completed і lapsed на сервері клієнт не володіє.»
...
the listing will contradict each other. `assemble.py` (`listing_solver_parameters`) replaces the
single `max_time_in_seconds` line with the deployed set and adds the stability bonus to the hint —
values from `services/recsys/src/hourwell_recsys/params.py`:
...
and `add_hint(model, previous_plan, stability_bonus=1e-4)`. No comments in the listing (owner,
2026-09-13); the paragraph below, placed after the sentence that announces the listing, carries
what the comments said and names every parameter:

> «Для кожної задачі створюється булева змінна розміщення y, змінна старту з доменом допустимих
> стартів F(τ) і необов’язковий інтервал, активний лише за y = 1; канальні літерали x зв’язують
> старт із конкретним слотом k і несуть у цільовій функції вагу, складену з цінності задачі, оцінки
> q̂ для її категорії в бакеті слота та множника терміновості. Закріплені задачі отримують обмеження
> y = 1 (умова C1), а обмеження AddNoOverlap реалізує (2.5); довжина інтервалу включає буфер між
> блоками, змінна кінця обмежена горизонтом K, а з цільової функції віднімаються м'які штрафи за
> відкладення критичних задач, перемикання контексту та фрагментацію (підрозділ 2.3). Підказка з
> попереднього плану є теплим стартом проти «трясіння»; оскільки сама підказка не зберігає порядку
> за однакової цільової функції, до її старту додається бонус стабільності в одну масштабовану
> одиницю (10⁻⁴ у вагових одиницях). Параметри розв’язувача – ті, що їх змінило вимірювання
> підрозділу 2.4: ліміт часу 1,5 с (anytime, NFR-P1), два робітники за двома закріпленими ядрами,
> вимкнене зондування та симетрійний передрозв’язок, критерій зупинки за відносним розривом 0,01 і
...
> «Нічний конвеєр виконується системним таймером на тій самій віртуальній машині в ЄС (щодня о 00:30
> UTC, з розсіюванням), у тому самому закріпленому контейнері, що обслуговує запити: (1)
> псевдонімізований експорт категоріальних та поведінкових ознак із Postgres за білим списком
> стовпців, який є даними, а не кодом, і звіряється тестом безперервної інтеграції проти реальної
> схеми (NFR-S3) – жоден рядок рівня події в міжкористувацьке навчання не потрапляє, бо об'єктами
> факторизації є комірки, а не події; (2) перенавчання ALS-факторизації бібліотекою implicit та
> переоцінювання кластерів k-середніх; (3) обчислення офлайн-метрик і оцінювальний бар'єр: нова
> версія публікується лише за непогіршення метрик; (4) публікація артефактів у Supabase Storage (ЄС)
> та рядок у `model_registry`; (5) Монте-Карло-оцінювання пропенсіті для трафіку семплінгу Томпсона
> (K = 32 вибірки). Конвеєр неперервної інтеграції GitHub Actions виконує той самий код на
> синтетичній когорті – на кожен запит на злиття і на кожне вливання, – щоб зміни конвеєра
> перевірялися без доступу до даних учасників.»
...
> задачами, огляди доступності на найбільшому масштабі шрифту, діалоги підтвердження та українські
> потоки. Апаратна перевірка (підрозділ 6.6) показала межу цього рівня свідчень: жоден із потоків не
> запускається на фізичному iPhone (Maestro не підтримує фізичні пристрої iOS), на реальному Android
> селектори вкладок не збігалися з жодним елементом, бо Android не додає до міток рядка «, tab», а
> одне твердження про формат дати містило екрановану послідовність у лапках YAML і не могло
> спрацювати. Три факти окреслюють межу переносності наскрізних наборів між платформами.»
...
виконано (підрозділ 5.2.3): роботу розповсюджують без облікових записів магазинів – збіркою APK для
Android; каналу для учасників з iOS немає, бо TestFlight потребує платного членства.»
...
> «Специфікацію, написану до розробки, цей конвеєр еталоном не вважає: розбіжність коду зі
> специфікацією, підтверджена вимірюванням, фіксується як зміна специфікації (підрозділ 6.8), а
...
| 6.6 | Верифікація на реальних пристроях: шість класів дефектів структурно невидимі без пристрою | item 62 in Ukrainian with §11.3's corrections applied, plus U23–U40's instrument discipline       |
...
> інструментами хронотипу є CSM і MCTQ (Senyk, Jankowski & Cholii [nn]); CSM – інший набір пунктів
> із власними межами (≤ 23 / ≥ 42), який потребував би власного виведення «бал → клас → приор». Ціну
> названо прямо: україномовний користувач частіше пропустить опитування, а пропуск є спроєктованим
> шляхом (клас INT з удвічі меншою силою приору), а не збоєм. Нижче зміст пунктів переказано
> українською для читача роботи; у застосунку вони показуються в оригінальному англійському
> формулюванні.»
...
> з'єднання; push операцій із курсором MMKV, миттєвий прохід атрибуції та pull в одному зверненні до
> `sync-resolve`; виявлення семантичного конфлікту; застосування панівного правила «факти важливіші
> за плани» – статус `completed` із прапорцем `conflict_flag`, а кортеж r = 1 записується з
> `excluded = true` і причиною `concurrent_external_conflict`: рядок у таблиці винагород є, а
...
> Postgres, Edge Fn `sync-resolve`. Вихідний код діаграми (`b1-sync.mmd`) зберігається в
> репозиторії системи; діаграму слід відрендерити та вставити зображенням._»
...
> «Специфікації системи є апріорними припущеннями, а не вихідними даними. У ході реалізації
> одинадцять із них спростовано вимірюванням; у кожному такому випадку переписували специфікацію, а
> не результат. Нижче – ці випадки, кожен із вимірюванням, яке його вирішило, і наслідком для
> системи.»
...
| Незважений replay незміщений на зрізі                    | зміщений на −0,6 / +0,7 в. п. за змінного \|A_m(x)\|                                  | зважений replay = SNIPS; методику (підрозділ 2.6.2) переписано                         |
