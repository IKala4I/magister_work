...
| In-app dialog (ADR-0021, 2026-09-06) | ✅ T title `header`, actions `button`, scrim hidden, card `accessibilityViewIsModal`, announce + focus on show, no `Alert.alert` anywhere | ✅ T ThemedText, scrolling body, stacked wrapping actions · ⬜ D | ✅ T spring ≤ 250 ms, collapses to 0 (`dialog.test.tsx`) · ⬜ D | ⬜ D (title → body → confirm → cancel)              | ✅ T 48 px rows · ⬜ D |
...
evidence added to `a11yAudit.test.ts`: no `Alert.alert(` in `app/` or `src/ui`; `dangerText`,
`primary` and `textPrimary` ≥ 4.5:1 on the elevated card in both palettes; `danger` never a
`color:` (3.76:1 as text — spec-conflicts L41). `dialog.test.tsx` pins roles, every way out
resolving as a cancel (button, scrim, Android back), the single slot, the announcement + focus
on show, the scrolling body, and the motion in both states (spring settles ≤ 250 ms; instant on
the first frame under reduced motion). Device-conditioned: TalkBack/VoiceOver order and
