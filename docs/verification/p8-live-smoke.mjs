...
function sql(query) {
  ...
  const out = execFileSync('supabase', ['db', 'query', '--linked', '--output-format', 'json', '-f', file], {
    cwd: repoRoot,
    encoding: 'utf8',
...
const { error: profileError } = await supabase.from('profiles').insert({
  ...
  working_hours: { [dayKey]: [0, 1440], mon: [540, 1080], tue: [540, 1080], wed: [540, 1080], thu: [540, 1080], fri: [540, 1080] },
...
const createOps = [
  { op_id: opId(), op_type: 'task_upsert', entity_id: TASK, base_version: null, payload: taskPayload },
  {
    op_id: opId(),
...
check('sync-resolve push → 200', s1.status === 200, `${s1.status} ${JSON.stringify(s1.body).slice(0, 160)}`);
check('acks: task create + fact + second task applied', s1.body?.acks?.map((a) => a.outcome).join(',') === 'applied,applied,applied', JSON.stringify(s1.body?.acks));
check('pull returns the profile and the task', ['profiles', 'tasks'].every((t) => s1.body?.pull?.some((r) => r.tbl === t)), JSON.stringify(s1.body?.pull?.map((r) => r.tbl)));
...
check('duplicate op replay is a no-op (both duplicate)', s2.body?.acks?.every((a) => a.outcome === 'duplicate'), JSON.stringify(s2.body?.acks));
check('nothing new to pull after the replay', (s2.body?.pull?.length ?? -1) === 0, String(s2.body?.pull?.length));

// --- a stale base_version comes back as conflict + server row --------------------------------
const stale = await sync(
  [{ op_id: opId(), op_type: 'task_upsert', entity_id: TASK, base_version: 7, payload: { ...taskPayload, title: 'stale edit', version: 8 } }],
  s2.body.cursor,
);
check('stale base_version → conflict with the server row', stale.body?.acks?.[0]?.outcome === 'conflict' && stale.body?.acks?.[0]?.row?.version === 1, JSON.stringify(stale.body?.acks?.[0]).slice(0, 200));
...
check('plan-request → planned (persist_plan RPC)', planned && plan.data.recommendations.length >= 1, plan.error?.message ?? JSON.stringify(plan.data).slice(0, 160));
console.log(`      plan-request round trip: ${planMs} ms (engine ${plan.data?.plan?.engine}, ${plan.data?.plan?.telemetry?.ef?.reason})`);
if (!planned) process.exit(1);
const recA = plan.data.recommendations.find((r) => r.task_id === TASK) ?? plan.data.recommendations[0];
...
check('rec A is displaced_pending (injected as the webhook would)', pendingRow[0]?.status === 'displaced_pending', JSON.stringify(pendingRow));
...
const factOps = [
  {
    op_id: opId(), op_type: 'event_append', entity_id: TASK, base_version: null,
    payload: { user_id: uid, type: 'focus_start', task_id: TASK, recommendation_id: recA.id,
      payload: { session_id: 's1', started_at: new Date(startMs).toISOString(), slot_start: recA.slot_start, planned_minutes: 60 },
      context: { tz: 'Europe/Kyiv' }, client_ts: startMs, local_day: localDay },
  },
  {
    op_id: opId(), op_type: 'task_upsert', entity_id: TASK, base_version: 1,
    payload: { ...taskPayload, status: 'done', done_at: endMs, version: 2, updated_at: endMs },
  },
  {
    op_id: opId(), op_type: 'event_append', entity_id: TASK, base_version: null,
    payload: { user_id: uid, type: 'focus_end', task_id: TASK, recommendation_id: recA.id,
      payload: { session_id: 's1', outcome: 'finished', started_at: new Date(startMs).toISOString(), ended_at: new Date(endMs).toISOString(), focused_ms: 55 * 60_000, planned_minutes: 60, est_minutes: 60 },
      context: { tz: 'Europe/Kyiv' }, client_ts: endMs, local_day: localDay },
  },
];
...
check('offline facts replayed: all applied', s3.body?.acks?.every((a) => a.outcome === 'applied'), JSON.stringify(s3.body?.acks));
check('reward pass ran and wrote a tuple', (s3.body?.rewards?.tuples_written ?? 0) >= 1, JSON.stringify(s3.body?.rewards));
const pulledA = s3.body?.pull?.find((r) => r.tbl === 'recommendations' && r.row.id === recA.id)?.row;
check('FACTS BEAT PLANS: rec A pulled as completed + conflict_flag', pulledA?.status === 'completed' && pulledA?.conflict_flag === true, JSON.stringify({ status: pulledA?.status, conflict_flag: pulledA?.conflict_flag }));
check('the meeting is pulled as a calendar_events row', s3.body?.pull?.some((r) => r.tbl === 'calendar_events' && r.row.title === 'Design review'), '');
check('the task is pulled as done (7→8 analogue: version 2)', s3.body?.pull?.some((r) => r.tbl === 'tasks' && r.row.status === 'done' && r.row.version === 2), '');
const tupleA = sql(`select reward, reason, excluded, excluded_reason from public.feedback_rewards where recommendation_id = '${recA.id}' and kind = 'outcome'`);
check('reward tuple stored EXCLUDED (concurrent_external_conflict), value kept for audit', tupleA[0]?.excluded === true && tupleA[0]?.excluded_reason === 'concurrent_external_conflict' && Number(tupleA[0]?.reward) === 1, JSON.stringify(tupleA));
...
if (recB) {
  sql(`update public.recommendations set status = 'displaced_pending', slot_start = now() - interval '3 hours', slot_end = now() - interval '2 hours' where id = '${recB.id}' and user_id = '${uid}';`);
  const s4 = await sync([], s3.body.cursor, 'foreground');
  const pulledB = s4.body?.pull?.find((r) => r.tbl === 'recommendations' && r.row.id === recB.id)?.row;
  check('counterfactual: rec B → displaced on sync', pulledB?.status === 'displaced', JSON.stringify({ status: pulledB?.status }));
  const tupleB = sql(`select count(*)::int as n from public.feedback_rewards where recommendation_id = '${recB.id}'`);
  check('counterfactual: NO reward tuple for the displaced block (H3)', tupleB[0]?.n === 0, JSON.stringify(tupleB));
} else {
...
check('concurrent syncs: lease serialises (200/200 or 200/409)', statuses.join('/') === '200/200' || statuses.join('/') === '200/409', statuses.join('/'));

// --- Google functions: fingerprints without credentials -----------------------------------------
const gc = await supabase.functions.invoke('gcal-connect', { body: { action: 'status' }, region: 'eu-west-1' });
check('gcal-connect status → connected:false', gc.data?.status?.connected === false, gc.error?.message ?? JSON.stringify(gc.data));
...
check('gcal-connect start without Google credentials → 503 not_configured', gs.status === 503, String(gs.status));
...
check('gcal-callback bounces to the app deep link', cb.status === 302 && loc.startsWith('hourwell://gcal-callback?status='), `${cb.status} ${loc}`);

// --- timing sample -------------------------------------------------------------------------------
const times = [];
for (let i = 0; i < 5; i++) times.push((await sync([], s3.body.cursor, 'poll')).ms);
times.sort((a, b) => a - b);
console.log(`      sync-resolve bare poll ×5: min ${times[0]} ms, median ${times[2]} ms, max ${times[4]} ms`);
