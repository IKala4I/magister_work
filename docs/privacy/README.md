...
| Credentials on the box                                                            | `DATABASE_URL` (pooler DSN — least-privilege `recsys_service` role shipped in P12 — migration + runbook §18; the DSN rotation to it is an owner step) and `HOURWELL_SERVICE_KEY`, in `~/hourwell/.env` (mode 600, owner-only user). Rotation procedure: runbook §11.                                                                                                                                                                                                             |
