...
| R4  | **VM compromise** → live DB credential + service key abuse                                                                                                        | L×H | §4 hardening; nothing at rest on the box; blast radius = model-state tables + transient requests; rotation runbook; least-privilege `recsys_service` DB role **rotated live 2026-08-31** (runbook §18): the service DSN can no longer read user content or bypass RLS | **L** (role rotation done 2026-08-31)        |
...
| ----------------------- | ----------------- | ---------- | ------- |
| Controller / researcher | Vladyslav Pershko | 2026-09-01 | signed  |

**Status at signature (2026-09-01):** this DPIA is signed as a **completed Art. 35
assessment of a system ready to process** — the field study it describes has **not
commenced** and no participant processing is underway (owner decision at signing: no field
study will run). The signature attests the assessment, not an authorization to begin
processing; any future enrollment requires re-reading this document against review
triggers 1–7 (§11) first.
