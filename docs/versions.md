...
| react-native-reanimated                            | 4.5.1    | + react-native-worklets 0.10.1 (v4 peer). First importer: the in-app dialog (2026-09-06). Verified against the docs that day: duration-based `withSpring({ duration, dampingRatio })`; jest = worklets mock (`react-native-worklets/lib/module/mock`) + `setUpTests()` in setupFilesAfterEnv |
...
| Expo SDK 57 patch drift (2)    | expo 57.0.19 · expo-router 57.0.18 · expo-notifications 57.0.16 · expo-constants 57.0.17 · expo-sharing 57.0.17 · expo-linking 57.0.9 · expo-font 57.0.3 · expo-secure-store 57.0.3 · @expo/metro-runtime 57.0.15                                                                                                                              | `npx expo install --fix` 2026-09-02 after expo-doctor failed CI on PR #37 (9 patch releases upstream since 2026-09-01); expo-doctor 21/21, jest 461. **The APK on the Pixel 7a remains the 57.0.18 build** — the hardware-pass numbers are measured on that binary; the end-of-pass rebuild picks the patches up                                                 |
| Expo SDK 57 patch drift (3)    | expo 57.0.20 · expo-notifications 57.0.17 · expo-router 57.0.19 · expo-sharing 57.0.18                                                                                                                                                                                                                                                         | `npx expo install --fix` 2026-09-05 after expo-doctor failed the TypeScript-gates job on PRs #45–#47 (4 patch releases upstream since 2026-09-02); expo-doctor 21/21, jest 519. **The APK on the Pixel 7a (build 5, 7c8f67c) remains on the previous patches** — every day-4/5 number is measured on that binary; the post-pass rebuild (build 6) picks these up |
| Build-6 fix batch (2026-09-05) | expo-modules-core 57.0.16 (`requireOptionalNativeModule` re-exported by `expo`; `expo-module-gradle-plugin`) · Android API 31+ `AlarmManager.canScheduleExactAlarms()` / `Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM` · @radix-ui/react-slot 1.3.3 (expo-router `Link asChild`: child props win the merge) · @shopify/flash-list stays 2.0.2 | Local Expo module `apps/mobile/modules/exact-alarm` (no new npm package; autolinked from `./modules`, Kotlin `Module` DSL — docs verified with ctx7 `/expo/expo` + `/websites/expo_dev_versions_sdk_notifications`: expo-notifications exposes no exact-alarm API). FlashList bump deferred until the blank card reproduces on build 6 (day-5 notes item 9).     |

## Post-P12 — iPhone pass (verified 2026-09-08)
