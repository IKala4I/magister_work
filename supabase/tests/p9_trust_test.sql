...
select results_eq(
  ...
  $$values ('dev-1', 'deep', 'MO', 'weekday', 'beta:deep.MO.weekday', 'correct', '2026-08-29T09:00:00Z'::timestamptz, 'client', null::timestamptz)$$,
  'the row carries the parsed cell, the label, the client timestamp, undelivered');

-- replay of the same op: duplicate, still one row
select is(
  (select r->>'outcome' from jsonb_array_elements(public.sync_replay('00000000-0000-4000-8000-000000000d01', '[{"op_id":"dev-1","op_type":"event_append","entity_id":null,"base_version":null,"payload":{"op_id":"dev-1","user_id":"00000000-0000-4000-8000-000000000d01","type":"belief_label","task_id":null,"recommendation_id":null,"payload":{"state_ref":"beta:deep.MO.weekday","label":"correct"},"context":{},"client_ts":"2026-08-29T09:00:00Z","local_day":"2026-08-29"}}]'::jsonb)) r),
...
select is(
  (select r->>'outcome' from jsonb_array_elements(public.sync_replay('00000000-0000-4000-8000-000000000d01', '[{"op_id":"dev-5","op_type":"event_append","entity_id":null,"base_version":null,"payload":{"op_id":"dev-5","user_id":"00000000-0000-4000-8000-000000000d01","type":"weekly_review_completed","task_id":null,"recommendation_id":null,"payload":{"week":"2026-W35"},"context":{},"client_ts":"2026-08-29T11:00:00Z","local_day":"2026-08-29"}}]'::jsonb)) r),
  'applied', 'a weekly_review_completed event is a plain fact');
select is((select count(*) from public.belief_labels), 2::bigint, 'plain facts never materialise labels');
...
select is((select count(*) from public.belief_labels), 2::bigint, 'A reads own labels (toggle state across devices)');
