...
**Status board:** P0 ✅ merged (PR #1) · P1 ✅ merged (PR #2) · P2 ✅ merged (PR #3; carry-over measurements done — NFR-P2 p90 = 1075 ms and NFR-A2 sweep 27/27, both on the iOS **simulator**: threshold met, device condition not — device re-check on the checklist below) · P3 ✅ merged (PR #4; adversarial pass found 3 MAJORs, all fixed + re-verified on the iOS **simulator**, Release build — see `docs/verification/p3-manual-verification.md`) · P4 ✅ merged (PR #5; cold-start math verified 3 ways incl. live smoke on the hosted project; adversarial pass: 1 MAJOR session-fixation + 10 MINOR, all fixed — `docs/verification/p4-manual-verification.md`; ⛔ open: Google OAuth consent screen, mailbox magic-link E2E) · P5 ✅ merged (PR #6; FastAPI service + CP-SAT planner per File 04 §1, exact ε-slice propensity, H3-distinct reward paths; adversarial pass: 2 MAJOR (top-m over chunk-only buckets → biased slice; excluded correction never rebuilt) + 11 MINOR, all fixed — `docs/verification/p5-manual-verification.md`; timing measured on a Mac only, 2 vCPU Space pending; ⛔ open: HF Space creation) · P6 ✅ merged (PR #7; `plan-request` edge function + arm A "heuristic + matched randomization" on TypeScript mirrors of the service pinned by a parity fixture; eligibility |A_m(x)| ∈ {2,3,4} with exact per-row propensity (owner decision, rate measured); Today screen; adversarial pass — see `docs/verification/p6-manual-verification.md`; learned path live + NFR-P1 warm p95 pending the HF Space) · P7 ✅ merged (PR #8; feedback loop end to end minus a live service: `attribute-rewards` EF with instant + 23:55-local daily modes (boundary in SQL, DST pgTAP-tested), stored-then-delivered tuples, blend SGD with River as oracle, duration estimator, Focus tab + block actions + lazy lapse scan + third-skip diagnostic; 290 jest + 98 Deno + 135 pytest + 32 pgTAP; adversarial pass 7 MAJOR + 14 MINOR, all MAJORs fixed; ⛔ open: RecSys host decision — ADR-0009 — after Hugging Face withdrew free Docker Spaces; Vault secrets for the cron tick) · **P7.1** RecSys hosting (ADR-0009 accepted: Oracle Always Free A1 in eu-marseille-1; deploy bundle + pull-based GHCR rollout + runbook + privacy/DPIA update; owner steps ⛔ 1–7 in HANDOFF, then the three measurements; **ADR-0011** accepted 2026-08-28 — option A, in-region by construction; P11 scope amended; **service live 2026-08-28** — all four deferred measurements done: day plan 487 ms p90 on the box, learned path p95 1.5 s end-to-end, `/feedback` 24/24 delivered via pg_cron, outage fallback in budget; threshold re-fitted 8·10³ → 3·10³ on the box; P7 `config.toml` entrypoint bug fixed) · **P8 ✅ merged (PR #16; ADR-0012: `sync-resolve` push-then-pull with a replay ledger, the three File 05 §2 conflict classes, client field-level merge, RLS-invoker pull, per-user lease, `persist_plan` RPC, `displaced_pending` resolved by the reward mapping (completed + `conflict_flag` + EXCLUDED tuple, or `displaced` with no tuple), Google Calendar (server-held OAuth, push channels + 5-min sweep, opt-in write-back) verified against a fake Google AND live against the owner's Google Calendar on 2026-08-29 (consent, push, 20-day event, displacement without a reward, all-day rule, disconnect), deferred wipe; 344 jest + 155 Deno + 85 pgTAP; File 05 §2 reproduced end to end incl. the counterfactual branch in a Deno scenario AND live on the hosted project (27/27); adversarial pass 3 MAJOR + 12 MINOR, all but one documented residual fixed — `docs/verification/p8-manual-verification.md` §4; before enrollment: Google consent screen Testing → In production)** · **P9 ✅ merged (PR #17; ADR-0013: belief labels = one prior's worth of pseudo-evidence on the named Beta cell, latest label in force, every delivery a full rebuild; `insights` EF with File 06 PAR from facts only; FR-40 heatmap on native Views with OKLCH + text alternative; FR-24 sheet applying the pick as a task op + decision fact; 149 pytest + 166 Deno + 23 pgTAP + 382 jest; adversarial pass 4 MAJOR + 10 MINOR, all MAJORs and 8 MINORs fixed — `docs/verification/p9-manual-verification.md` §4; live smoke 31/31 on 2026-08-30 after the owner pushed the migration — the full label round trip verified live; one live finding fixed: labels no longer switch the learning-mode badge off)** · **P10 ✅ built (PR #19; ADR-0014: FR-50 local reminders with the hard ≤5/day cap enforced by a pure planner over a conservative delivered-ledger (storm-tested), FR-26 evening ritual (Accept plans tomorrow), FR-32 `notification_response` facts, FR-42 `export-data` + `delete-account` with the cascade proven by pgTAP (36/36 on the linked schema) and retention fixed at 30 d inactivity, SDK opt-outs, a11y source audit with three real AA fixes, Maestro sweep + device-pass script, perf table from Node — NFR-P3 holds for the CRUD API (88/82 ms p95), not for the composite function round trips (477 ms); 457 jest + 184 Deno + 149 pytest + 36 pgTAP; live smoke **25/25** on the migrated project 2026-08-30 — the full export/erasure round trip incl. the dead-session check; two smoke findings fixed: server-side session verification on the account functions, one shared db-query parser)** · **P11 ✅ built (PR #21; ADR-0015: nightly training on the EU VM — NFR-S3 whitelist-as-data (pgTAP-pinned CHECK rule + client-vocabulary pin), EB prior refresh behind a held-out log-loss eval gate with promoted-version selection in `instantiate_user_priors` (closes ADR-0005 §6), ALS on decayed cell aggregates + silhouette k-means + closed-form fold-in ≥30 with unvisited-cell-only refresh (invariant 5 in SQL), K=32 MC propensity backfill through the service's own scoring (path dep), OPE family with hard slice discipline (replay refuses rows without exact p = 1/|A_m(x)| + logged A_m(x); IPS/clip/SNIPS/DR on slice AND MC-labeled TS traffic; ESS<100 = NON-EVIDENCE on every row) proven to RECOVER closed-form truth on synthetic ground data; aggregate report (min cell 5) incl. per-arm×phase PAR from facts only (H2); study mode (enroll_participant ABAB/BABA + blocked-randomization audit + the G6 EU/EEA gate); `train.yml` synthetic-only e2e (G3) + arm64 image with in-image ALS smoke; 74 training pytest + 149 recsys + pgTAP 26/26 (linked, rolled back) + 461 jest + 187 Deno; adversarial pass 6 MAJOR + 13 MINOR, ALL fixed same day — `p11-manual-verification.md` §4; ⛔ before live verification: migration push, VM service-role key + ARCHIVE_SALT, install.sh re-run)** · **P12 ✅ built (release prep, phase/P12-release-prep): Art. 35 DPIA drafted for owner signature (12-risk table, none high → no Art. 36; transfers annex = ADR-0011 §2); EAS build profiles (eas.json, remote versions); store pack — listing copy within verified limits, data-safety answers from the DPIA, privacy-policy draft, formal name search CLEAN (no Hourwell app or indexed mark; official-register pass = 10-min owner step); real README; runbook: duplicate §10–§12 renumbered → §13–§15, new §16 job triage / §17 registry rollback / §18 role activation; least-privilege `recsys_service` role (migration + 20 pgTAP, compose fallback override — behaviour unchanged until owner rotates the DSN); v0.1.0 CHANGELOG rollup; corrections-rollup.md (47 items grouped per draft chapter); store economics re-raised: Play $25 now, Apple $99/yr only when iOS participants are real (TestFlight is the only viable 8-week iOS channel) — owner decision; ⛔ DPIA sign-off, dev accounts, migration push + role DSN rotation.** Hardware pass still gates device-conditioned claims.
...
_Status (2026-08-29):_ built per ADR-0013 — corrections are `belief_label` facts → trigger →
`/labels` → full rebuild (measured in the service tests); heatmap from `/insights`; the sheet
reads `plans.telemetry.infeasible`. Live label round trip pending the ⛔ migration push.
...
_Status (2026-08-30):_ complete — local notifications planned by a pure function over a
conservative delivered-ledger (cap storm-tested); evening ritual with Accept/Adjust; `export-data`

- `delete-account` (self/operator/retention) with the cascade proven by pgTAP on the linked
  schema; retention fixed (anonymous 30 d inactive); a11y audit test + three AA fixes + Maestro
  sweep + `scripts/device-pass.sh`; perf table from Node (NFR-P3 ✅ CRUD API, ❌ composite function
  round trips — revisit). Live: migration applied by the owner 2026-08-30; smoke **25/25** —
  export document + self-erasure + dead session + zero rows + audit, observed service-side.
...
**P11 — Training pipeline + OPE + study mode.** _(amended 2026-08-28 by ADR-0011 option A —
participant data never leaves the EU and never reaches CI)_
Nightly training runs **on the EU VM** (`training/` container next to the service, systemd timer
in the keep-busy slot): pseudonymized categorical export (CI-tested query — NFR-S3), ALS fit,
k-means clusters (silhouette), fold-in ≥30 outcomes, empirical-Bayes prior refresh
(`kind='priors'` registry rows), eval gate, artefacts to **Supabase Storage (EU)**
...
| 4   | **iOS-first** development; Android compiles in CI; Android device pass at P10. Store economics ($25 vs $99/yr) is a P12 decision — re-raise there only. **Decided 2026-08-31: neither account — store pack prepared-but-unsubmitted; Android sideload installs; no iOS participant channel (docs/store/metadata.md §7).**                                            |
...
owner-run work is the account-free hardware pass (**Android closed 2026-09-05**, days 1–5 on the Pixel 7a; iOS out of scope by the store decision), the **post-pass fix batch → build 6** (2026-09-05: ADR-0019 implemented, the blank-card clip, button roles, the exact-alarm prompt — device re-check in `docs/verification/device-pass/android-20260905-1725-build6/`), the simulation study (**done 2026-09-05** — pre-registered in git, run once on `ec1b869`, compared prediction by prediction in `docs/study/simulation-results.md`) and the thesis-text pass; enrollment-conditioned
gates are retired unless the decision reverses. **Post-P12 localisation (ADR-0023, 2026-09-09):** a Ukrainian catalog and a language switch in Settings; the English boundary (the rMEQ instrument, the closed client↔server vocabulary, the user's own words, the export, diagnostics) is decided, stated in the app and written to be quoted in the thesis; FR-11 parses both languages and the "chrono-node is English-only" note is retired. Decision 6's "Ukrainian = add-a-file later" is done. **Post-P12 motion (ADR-0022, 2026-09-09):** File 02 §3.4 closed on the plan surface — two transitions (the rows settling after Done / Skip / I did it; a moved block travelling or the list scrolling to it), verified on both phones with NFR-P2 unchanged; spec-conflicts L42 closed. **Post-P12 corrections rollup (2026-09-09):** `docs/thesis/corrections-rollup.md` rewritten as the paste-ready edit set — all 63 items carry the Ukrainian text the draft should read, grouped by the draft's chapters and tagged Ф / П / С, with every number carrying its measurement condition; three chapters identified as structural rewrites (Розділ 5, a new Розділ 6, and §1.4's research-gap argument); forty findings no correction covered (U1–U40); six worklist entries amended. Next: writing the thesis text from it. docs/HANDOFF.md carries the state._
