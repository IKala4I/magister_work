...
**Amendment (2026-09-06; spec-conflicts M9, M16).** "1 slot/day" is the probability ε = 1 per plan, not a guaranteed daily experiment: eligibility is $|\mathcal{A}_m(x)| \in \{2, 3, 4\}$ with the exact per-row $p = \varepsilon/|\mathcal{A}_m(x)|$ (ADR-0008 §1), and the measured rate is ≈ 4.3 experiments per user-week on plain weeks, 1.1–2.4 on heavy weeks. The slice's cost, measured in simulation, is 0.03–17 pp of completion on the randomized blocks (≈ 0–4 pp at arm level) — the draw is identical in both arms, its cost is not.
...
- **Anytime:** `max_time_in_seconds = 1.5`; best feasible solution returned (CP-SAT is anytime), meeting NFR-P1 on 2 vCPU. **Measured (2026-08-28, 2026-09-02/03):** day plans (12 tasks) OPTIMAL 20/20 at p90 487 ms on the 2-core Oracle A1; on a 14-task day the learned path fell to the heuristic fallback 1 of 10 times before ADR-0018 (gap limit + no-improvement stop) and 0 of 10 after; NFR-P1 itself is restated in File 02 (≤ 6.0 s device end-to-end).
