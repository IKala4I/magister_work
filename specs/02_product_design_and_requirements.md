...
1. **Physics, not flourish:** spring-based transitions ≤ 250 ms; reduced-motion honored (NFR-A2). **Amended (2026-09-06, spec-conflicts L42):** until the in-app dialog (ADR-0021) the shipped app had no transitions at all; the dialog is the first conforming one (200 ms in, 120 ms out, reduced motion → 0 ms on the same path); the remaining surfaces are tracked in `docs/decisions/revisit.md`. **Amended (2026-09-09, ADR-0022, spec-conflicts L42 closed):** the plan surface conforms on the two interactions where motion tells the user what changed and where — Done / Skip / I did it (the rows below settle into the gap on `springs.standard`, 200 ms) and Move (the block travels to its slot, or the list scrolls to it and the arrived card settles on `springs.emphasized`, 250 ms); measured on the Pixel 7a at 11 / 10 / 14 / 12 frames at 60 fps (Done / Skip / I did it / on-screen move) and, for an off-screen move, a 17-frame scroll followed by a 7-frame arrival settle, hitch-free on the iPhone 12; reduced motion registers no transition on the same `resolveMotion` path (one frame on Android). Motion that would only say "something happened" (the plan-applied entrance) was judged decoration and left out; drag physics stay out (M10).
